...
- **Nhãn một nguồn.** Chữ hiển thị của một trường lấy từ Registry; nhãn mã trạng thái (`ttxly`/`tthai`) lấy từ `statusLabels.ts` (chỉ gán nhãn cho mã ĐÃ KIỂM CHỨNG — nguyên tắc bằng chứng). Cấm gõ chuỗi nhãn rời trong màn (nguồn gốc lệch "Tổng TT" vs "Tổng thanh toán").
