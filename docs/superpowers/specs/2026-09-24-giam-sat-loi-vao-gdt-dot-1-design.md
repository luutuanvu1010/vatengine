...
| QĐ-4 | Canary chạy **mỗi giờ, một lời gọi, một nguồn** (cron riêng); MST không tồn tại `0000000000`, captcha cố ý sai; **không** giải captcha, **không** né chặn, **không** xoay IP | Thiết kế theo ranh giới Hiến pháp |
...
- `canaryHealth.ts` (thuần, không I/O): `CanaryState = { lastVerdict?, consecutiveBad, alerted, since? }`; `nextCanaryHealth(prev, result)` trả `{ state, alert: { kind: "chan" | "drift" | "loi_lien_tiep" | "hoi_phuc", ... } | null }` theo QĐ-5. `HEALTHY_CANARY` là trạng thái ban đầu; **lần đầu tiên** không có trạng thái trong DO (`undefined`) → phát alert `kind: "bat_giam_sat"` một lần (để kiểm cái chuông khi deploy).
...
- sync-worker: `nextCanaryHealth` — `WAF_BLOCKED` lần đầu → alert `chan`, lần hai → null; `OK` sau chặn → `hoi_phuc`; `TIMEOUT` ×2 → null, ×3 → `loi_lien_tiep`; trạng thái `undefined` → `bat_giam_sat` một lần. `isEgressBlocked` true với `WAF_BLOCKED`. `runCanary` gọi đúng thứ tự load → save → emit. Sink: gửi Telegram khi có cấu hình; thiếu cấu hình → warn + không ném; nội dung không chứa bot token.
...
- **CHƯA KIỂM CHỨNG — độ ổn định dài hạn của 401 với MST giả:** GDT có thể đổi cách trả cho MST không tồn tại (vd 400 "MST không hợp lệ") → canary báo `DRIFT` giả. Xử lý: tin `DRIFT` mang mã + thông điệp để người đọc quyết nhanh; nếu xảy ra, cập nhật bảng phân loại có kiểm chứng.
