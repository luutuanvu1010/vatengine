...
| H-B.6 | CODE | **XONG + LIVE PRODUCTION (2026-07-20)** — (a) DLQ consumer (`dong_bo_that_bai` + audit CRITICAL + replay thủ công `/dlq/replay`) + (b) EgressHealth-gate GEO_BLOCKED (scheduled + consumer, `blockedAction` tôn trọng trần→DLQ). Đã **merge PR #18** → `feat/cloudflare-stack-u0`; **migrate 0008** áp Neon prod (RLS ENABLE+FORCE verified); **deploy `vat-sync-worker`** (Version 46769ea4, vat-sync-dlq consumer live). Hoãn (c) → H-B.6c. **NỢ ĐÓNG NỐT:** bật Cloudflare Access cho `/dlq/replay` (Phần A hardening đã xong: workers.dev off + custom domain, PR #19 chờ merge). Xem `HANDOFF-2026-07-20.md`. | PR #18, #19 |
| H-B.6c | CODE | Chưa bắt đầu — DO global-egress quota tổng; HỢP NHẤT BACKLOG #1 (rate-limit toàn cục GDT). Ghi chú từ H-B.6: replay tạo hàng ledger MỚI mỗi lần (append-only, thiếu khoá dedup DLQ-re-entry). | — |
