...
- [ ] **U15.5** Đối chiếu `GET /reconcile` — 4 loại finding + tóm tắt; gap nhãn "nghi thiếu". ⚠️ **Màn ĐANG ẨN** khỏi bảng điều khiển (cờ `SHOW_RECONCILE=false`, `apps/web/src/lib/featureFlags.ts`): menu + route tắt, mã màn và `@vat/reconcile` giữ nguyên. Lịch sử: ẩn 22/07 → bật 29/07 (đo production: 15/31.807 hóa đơn lệch thuế) → **tạm ẩn lại 29/07** để nghiên cứu thêm trước khi cho khách thấy. Nghiệm thu màn này hoãn tới khi bật lại cờ; điều kiện bật lại ghi ở `docs/BACKLOG-y-tuong-va-de-xuat.md` §"[2026-07-29] Đối chiếu — tạm ẩn".
