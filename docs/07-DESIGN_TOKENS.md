...
- **Đỏ THỨ BA — `--notify-*`, cho SỐ ĐẾM NỔI TRÊN NÚT** (chủ dự án chốt 2026-07-28; **mở rộng nghĩa 2026-07-29**). Con số nhỏ ở góc nút, báo "có N thứ đáng chú ý". Không phải lỗi (lỗi là `--danger-*`), không phải thương hiệu (`--brand-*`) — nó là quy ước thị giác quen thuộc từ ứng dụng di động. Tách token riêng để đổi sắc đỏ thông báo KHÔNG kéo theo màu cảnh báo lệch thuế, và ngược lại.
  - Ban đầu định nghĩa hẹp là "số đếm CHƯA ĐỌC". U40 bỏ hẳn khái niệm đã-đọc (cảnh báo nay dẫn xuất từ dữ liệu × bộ lọc, không lưu trạng thái) nhưng vẫn dùng đúng chấm này cho "số hóa đơn bị sửa ở kỳ khác". Nghĩa đúng của token là **số đếm cần chú ý**, có hay không có trạng thái đọc đều được.
  - **Không** dùng `--notify-*` cho bất kỳ mục đích nào khác.
