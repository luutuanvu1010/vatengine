...
## [2026-09-24] Sự cố GDT chặn đăng nhập hai tuần (WAF `request-id`) — việc còn nợ sau khi vá

**Bối cảnh/bằng chứng:** audit production 14 ngày: 13 lượt `dang_nhap_thue_that_bai`, 0 lượt thành công, tất cả cùng lý do GDT trả `403 "Hệ thống phát hiện hành vi không hợp lệ. Yêu cầu đã bị chặn."`; lần đăng nhập GDT thành công cuối 2026-09-09 09:34; lần đồng bộ cuối 2026-09-09 10:32; 0/11 tài khoản thuế còn token. Curl tái lập: thiếu header `request-id` → 403; có → 401 "Mã captcha không đúng." Đã vá ở `packages/gdt-client/src/http.ts` (gắn `request-id` tập trung) + API trả `422 gdt_tu_choi` thay vì `401` (web coi 401 là hết phiên ứng dụng → đăng xuất oan). Contract test `authenticate.contract.test.ts` nay có ca không cần credential canh WAF.

### Còn nợ
1. **CHƯA KIỂM CHỨNG từ biên Cloudflare.** Bằng chứng mới có từ máy dev (curl + contract test qua Node fetch). Chặn theo header chứ không theo IP (curl có UA trình duyệt vẫn 403; có `request-id` không UA vẫn qua), nên kỳ vọng Workers cũng qua — nhưng phải **xác nhận sau deploy**: người dùng đăng nhập lại một lần, rồi `select hanh_dong, chi_tiet from audit_log where hanh_dong like 'dang_nhap_thue%' order by tao_luc desc limit 5`. Ưu tiên **Cao** (hệ thống đang đứng từ 10/09).
2. **Phát hiện sớm hơn:** sự cố kéo dài 14 ngày vì contract test/probe egress chỉ soi `/captcha` (vẫn 200). Đề xuất: cron sync-worker khi thấy **0 tài khoản có token còn hạn toàn hệ thống** trong > 24h thì phát cảnh báo (audit CRITICAL / kênh chủ dự án), và lịch chạy `make test-contract` định kỳ (U13) bao gồm ca WAF mới. Ưu tiên **Cao**.
3. ~~**Thông điệp lỗi ở màn Kết nối tài khoản thuế còn chung chung:** mọi lỗi ≠ 409 đều hiện "Captcha hoặc mật khẩu không đúng" — kể cả 502 `token_shape_unexpected`, mất mạng, hay GDT chặn WAF.~~ **ĐÃ GIẢI QUYẾT bởi U43 Task 6/7** (2026-09-24): API tách `503 gdt_chan` khỏi `422 gdt_tu_choi`, web ánh xạ đủ bốn nhánh (sai captcha / GDT chặn / mất mạng / lỗi khác) và không xin captcha mới khi bị chặn.
4. **Portal còn gửi `Action` và `End-Point`** (interceptor axios). Hiện chỉ `request-id` là cần (kiểm chứng 24/09). Nếu GDT siết thêm, thêm hai header này cùng chỗ (`withRequestId` trong http.ts). Ưu tiên **Thấp** — ghi để khỏi tìm lại.
5. **Tài liệu kế hoạch cũ** (`docs/plans/U14-design.md`, `U14-plan.md`, `HANDOFF-sau-U14`) vẫn ghi "401 khi GDT từ chối" — giữ nguyên làm lịch sử; hợp đồng sống là `docs/06-BINDING_MAP.md` (đã sửa).
...
7. **Lỗ "Kỹ thuật đã được báo" — 503 `gdt_chan` có thể KHÔNG ai được báo.** Ưu tiên **Cao**. *(Nguồn: review cuối U43, 2026-09-24.)* Nhánh 503 kích hoạt bởi chữ ký WAF trong phản hồi cho **request CỦA TENANT**. Nếu WAF (F5 ASM) bắt theo **nội dung thân request** — ví dụ ký tự đặc biệt trong mật khẩu thuế thật của một doanh nghiệp — thì tenant đó nhận 503 kèm câu "bộ phận kỹ thuật đã được báo", trong khi canary (payload cố định, MST giả `0000000000`) vẫn `OK` ⇒ **không chuông nào kêu**. Biện pháp 4 "gom audit đăng nhập cửa sổ 24 giờ" của đợt 2 chính là thứ bịt lỗ này (N thất bại `waf_blocked` trên nhiều tenant = chữ ký sự cố thật). Trước mắt đã có log CRITICAL `gdt_waf_blocked_login` ở `apps/api/src/routes/taxAccounts.ts` làm vết đối chiếu — nhưng log **không tự gọi ai**.
8. **403 thân HTML ⇒ web vẫn hiện đúng thông điệp sai mà U43 sinh ra để diệt.** Ưu tiên **Cao**. *(Nguồn: review cuối U43, 2026-09-24.)* `authenticate()` ở nhánh `!bodyLaJson` ném `GdtError` **không kèm `httpStatus`** ⇒ `isWafBlocked` false ⇒ API trả `422 gdt_tu_choi` ⇒ web báo "Captcha hoặc mật khẩu không đúng" **và xin captcha mới** ⇒ người dùng thử đi thử lại, dồn thêm lượt vào WAF. Sửa tối thiểu: truyền `res.status` + khoảng **200 ký tự đầu** thân thô vào `GdtError`; **nhớ cắt ngắn trước khi ghi `audit_log.chiTiet`** (thân HTML dài sẽ phình audit và `maskSensitive` không che được chuỗi tự do).
9. **Hai điểm nhỏ của lớp canary** *(nguồn: review cuối U43)*. (a) `CanaryVerdict` chưa có nhãn `RATE_LIMITED`: 429/5xx ở bước `authenticate` rơi vào `DRIFT` (báo NGAY) trong khi cùng lỗi đó ở bước `captcha` lại là `ERROR` (ngưỡng 3) — bất đối xứng về nhiễu, dễ báo động giả; ưu tiên **Trung bình**. (b) Một cờ `alerted` duy nhất cho cả đợt: đang trong đợt `loi_lien_tiep` đã báo mà sau đó GDT chuyển sang chặn thật (`WAF_BLOCKED`) thì tin `chan` bị im. Gợi ý rẻ: đổi `alerted: boolean` → `alertedKind?: CanaryAlertKind` và báo lại khi kind đổi — trạng thái nằm trong Durable Object nên **không cần migration**; ưu tiên **Thấp**.
...
**Vì sao im lặng 14 ngày:**
- Probe egress mỗi 15 phút (`apps/sync-worker/src/index.ts`) chỉ GET `/api/captcha` — WAF không áp lên captcha nên probe luôn OK. `classify()` (`packages/gdt-client/src/transport.ts:31`) xếp MỌI 403/451 vào `GEO_BLOCKED` → nếu WAF chặn cả captcha, hệ thống hiểu nhầm là chặn địa lý và rơi sang relay VN, nơi cũng bị chặn y hệt (chặn theo header).
- CI contract cron 02:00 UTC (`.github/workflows/ci.yml`) không có secret → chỉ `captcha.contract` chạy; authenticate/invoices/detail/exportXml đều skip; fail chỉ có thông báo mặc định GitHub. (Bản vá 24/09 đã thêm ca authenticate không cần credential — chuông này nay có tiếng.)
- Cron đồng bộ: `enumerateDueAccounts` (`apps/sync-worker/src/schedule.ts:57`) chỉ lấy tài khoản `token_het_han > now`. 0 tài khoản → cron "hoàn thành" với 0 việc, không đếm, không log, không audit. Im lặng trông y hệt thành công.
- Audit có `dang_nhap_thue_that_bai/thanh_cong` nhưng không gì gom theo cửa sổ thời gian.
- CRITICAL của probe chỉ là `console.error` (`apps/sync-worker/src/deps.ts:60`). Telegram bot có sẵn (`apps/api/src/thongBao/telegram.ts`) nhưng chỉ báo tenant đăng ký mới và sync-worker không với tới.
- `GdtContractDriftError` trong sync-worker: `classifyFailure` (`packages/sync/src/sync.ts:217`) không có nhánh drift → coi là `transient`, retry rồi breaker per-tenant, lặp mãi không báo ai.
