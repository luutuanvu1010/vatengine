...
3. **Thông điệp lỗi ở màn Kết nối tài khoản thuế còn chung chung:** mọi lỗi ≠ 409 đều hiện "Captcha hoặc mật khẩu không đúng" — kể cả 502 `token_shape_unexpected`, mất mạng, hay GDT chặn WAF. Nên tách: `422 gdt_tu_choi` → sai captcha/mật khẩu; còn lại → "Không kết nối được với Tổng cục Thuế, vui lòng thử lại sau" (+ mã lỗi để báo hỗ trợ). Ưu tiên **Trung bình**.
