...
| Gói 3 — hạ tầng bucket công khai | ⏸ **CỔNG DỪNG 2** — Claude xong phần mã, chờ chủ dự án dựng hạ tầng | `20cee3f` | Binding `CHIA_SE` + `Env` + dummy test. Lệnh + checklist ở `U37b-goi3-ha-tang-bucket.md` (cú pháp lấy từ wrangler 4.110 đang cài, không chép trí nhớ). Cần **Zone ID** của tourdao.vn. ⚠️ Bước dễ sót nhất: xác nhận `r2.dev` ĐANG TẮT |
| Gói 4 — phát hành gói | ⏸ | — | |
| Gói 5 — thu hồi + audit | ⏸ | — | |
| Gói 6 — giao diện | ⏸ | — | |
| Gói 7 — tài liệu | ⏸ | — | |
