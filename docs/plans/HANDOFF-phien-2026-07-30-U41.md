...
1. **Trạng thái:** 9 commit (`eeb5ee0`→`2136940`) đã đẩy, **CI xanh** (`5950017`); **`vat-web` đã deploy** — Version ID `f54c0e42-21c0-4042-8fda-1ad6f7677041`, bundle `index-D69GxB-7.js` trên `vatengine.tourdao.vn`. Không đụng DB / sync-worker / api (thuần frontend, **không migration** ⇒ quay lui = deploy lại bản `vat-web` trước đó).
...
3. **Việc còn nợ:** ⚠️ **`changelog.ts` CHƯA có mục U41** — người dùng không được báo về thay đổi lớn này; thêm mục ⇒ phải deploy lại `vat-web` (changelog nằm trong bundle). Chưa ai soi Tổng quan trên **production với số liệu thật** — đo truy vấn nặng nhất (`/invoices/summary` không lọc ngày, tenant 32.603 hóa đơn) là 19 ms nóng / 392 ms nguội **thẳng vào Postgres**, chưa tính đường qua Worker; nếu chậm, gỡ khối "Chỉ số đã đo được" là đủ, không ảnh hưởng phần khác. Chưa chạy review chéo `dod-auditor`. `?xem-thu=1` **chưa chặn `/goi-chia-se`** (lọt ra API thật).
