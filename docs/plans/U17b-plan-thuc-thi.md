...
> **ĐÍNH CHÍNH 2026-07-20 (chủ dự án duyệt) — gate audit đổi từ `row?.password_hash` sang `row`.** Kế hoạch trên (đã giữ nguyên verbatim ở trên, không sửa) dùng `row?.password_hash` làm điều kiện "tenant có thật" để quyết định có ghi `login_fail_chua_duyet` hay không. **Lỗ hổng đo được:** U17b Task 5 (`dangKy.ts`) tạo tenant tự đăng ký với `nguoi_dung.password_hash` **luôn NULL** (đặt mật khẩu là việc của luồng sau, ngoài phạm vi U17b) — nên với đúng biểu thức gốc, `row?.password_hash` **luôn false** cho quần thể tự đăng ký, khiến nhánh `login_fail_chua_duyet` không bao giờ đạt được cho chính quần thể nó được sinh ra để phục vụ, và **không audit nào** (kể cả `that_bai`) được ghi cho họ khi đăng nhập lúc chưa duyệt. Test gốc ở Step 1 không bắt được vì nó seed tenant qua `seedUser` (luôn gán một mật khẩu thật) — không đi qua đường đăng ký công khai thật. Probe xác nhận: tenant tạo qua `POST /dang-ky` thật rồi thử đăng nhập → audit chỉ có `["dang_ky"]`, không có hàng `dang_nhap_saas` nào. **Quyết định của chủ dự án (2026-07-20):** đổi gate thành `row && row.tenant_trang_thai !== "active"` — "tenant có thật" đúng nghĩa là tìm được hàng (`row` tồn tại), không phụ thuộc đã có mật khẩu hay chưa; giữ nguyên biểu thức 401 gốc, giữ nguyên vị trí verify PBKDF2 (luôn chạy đúng một lần), `recordFailure()` (luôn chạy uniform), và đường `settle()` off-critical-path — chỉ đổi GIÁ TRỊ truyền vào audit, không đổi luồng gọi. Xem `apps/api/src/routes/auth.ts` (bình luận tại chỗ) và test mới `apps/api/test/integration/auth.trangThai.test.ts` (describe "tenant tự đăng ký qua POST /dang-ky thật").
