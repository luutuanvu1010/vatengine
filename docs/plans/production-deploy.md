...
>
> **🔴 SỰ CỐ MIGRATE — `drizzle-kit migrate` báo thành công nhưng KHÔNG áp gì (phát hiện + vá ngay trong lượt deploy này, đã ghi nhớ dài hạn).** Chạy `drizzle-kit migrate` cho `0017_u35_lich_su_thay_doi_hoa_don.sql` lên Neon production: in `[✓] migrations applied successfully!`, không lỗi. Hậu kiểm trực tiếp (script `pg` độc lập, không qua drizzle) cho thấy **bảng/cột/trigger mới KHÔNG hề tồn tại** — lệnh đã im lặng không làm gì. **Nguyên nhân:** migration 0017 bị xoá-tạo-lại giữa phiên (vá lỗi coverage `packages/db`, xem `U35-tien-do.md`); `drizzle-kit generate` đóng dấu `when` bằng đồng hồ hệ thống của sandbox — đồng hồ này lệch, tạo ra `when` **nhỏ hơn** `when` của migration 0016 liền trước trong `_journal.json`. `drizzle-kit migrate` xác định "migration đang chờ" một phần theo thứ tự `when`; một `when` không tăng dần khiến nó bỏ qua migration mà vẫn báo thành công vô điều kiện. **Vá:** sửa tay `when` của mục idx:17 thành `0016.when + 60000` (đúng nhịp cách quãng ~60s đã dùng giữa các migration cùng ngày khác trong journal), commit riêng `097281f`. Chạy lại `migrate` — lần này tốn thời gian rõ rệt hơn hẳn (nhiều khung xoay hơn, dấu hiệu DDL thật đang chạy) — hậu kiểm lần 2 xác nhận đủ: bảng `lich_su_thay_doi_hoa_don` + `bo_dem_phien_ban`, cột `lan_dong_bo.so_phien_ban`, RLS ENABLE+FORCE cả hai bảng mới, trigger `hoa_don_ghi_lich_su_thay_doi_trigger` trên `hoa_don`, `drizzle.__drizzle_migrations` mới nhất `id=18`. **Bài học ghi vào memory (`drizzle-migrate-silent-skip-clock-drift`): không bao giờ tin thông điệp thành công của `drizzle-kit migrate` một mình — luôn hậu kiểm trực tiếp DB sau mỗi lần migrate thật, đặc biệt sau khi một migration bị xoá-tạo-lại giữa phiên.**
>
> **(0) Precheck Hyperdrive** = `vat_app` (không phải `neondb_owner` superuser), caching disabled — xác nhận qua `wrangler hyperdrive get 1011ff82e7154531883f0f2e62d344f0` trước khi migrate. **(1)** `make migrate` áp `0017` (sau khi vá sự cố ở trên) — hậu kiểm 5/5: 2 bảng mới, cột mới, RLS enable+force, trigger, `id=18`. **(2)** Deploy theo đúng thứ tự luật `deploy.md` + `deploy-thu-tu-va-grep-bundle`: `vat-sync-worker` (consumer hiểu message mới trước — Version `df11cae5-5a43-41c0-90fa-f4efb684eb09`, đủ 2 DO + 2 cron + producer/consumer 2 queue) → `vat-api` (producer — Version `d398e86d-2a1a-4217-8358-02555ac1bb9a`, đủ binding HYPERDRIVE+R2+DO+Queue) → build SPA `VITE_API_BASE=/api vite build` (bundle `index-BZWfzj7Q.js`) → **grep bundle TRƯỚC deploy** xác nhận có `Hóa đơn vừa thay đổi` + `invoices/changes` + `mark-read` (tính năng mới thật sự nằm trong bundle, không bị stale) → deploy `vat-web` (Version `25007f08-58f3-4d19-af23-3b096324adec`). **(3) Smoke production 8/8:** `/` → **200** nạp đúng bundle mới; bundle LIVE **byte-for-byte giống hệt** bundle build local (`diff` rỗng) và chứa đủ 3 chuỗi tính năng mới; `/api/health` → **200**; `/api/invoices` không token → **401**; `GET /api/invoices/changes` không token → **401** (endpoint mới tồn tại thật, không 404); `vat-api` không có route công khai trực tiếp (đúng thiết kế ẩn API); `vat-sync-admin.tourdao.vn` → **302** (Access sống); đủ security header (CSP, X-Content-Type-Options, X-Frame-Options). **Kiểm chứng bằng mắt trên production — chủ dự án xác nhận 2026-07-27:** đã đăng nhập tenant thật, thấy badge "Hóa đơn vừa thay đổi" hoạt động đúng và cột Thuế suất/Tiền thuế trong file xuất hiển thị đúng — chủ dự án xác nhận trực tiếp trong phiên ("ok đã kiểm, đã thay đổi"), không phải suy đoán của Claude. Changelog người dùng cuối: `v1.9` (`apps/web/src/lib/changelog.ts`, commit `0dfafd1`). **Đồng bộ nguồn sự thật (đã xong):** nhánh `claude/u35b-u35-sequential-4d3ad9` push lên origin + fast-forward thẳng `origin/feat/cloudflare-stack-u0` (`4d07abe..c88273b`, sau đó `..c400657` kèm ADR-0008) — không lệch trục. Sự cố migrate ở trên được chốt thành quyết định vận hành lâu dài ở `docs/adr/0008-hau-kiem-bat-buoc-sau-drizzle-migrate.md` + bổ sung `.claude/rules/deploy.md`.
>
> **Nhật ký deploy Danh sách hóa đơn — bỏ bảng + trả lại chọn ngày — 2026-07-23 (đã kiểm chứng):** Commit thẳng trên trục `feat/cloudflare-stack-u0` (`e5d1e66`), push origin (`4cf20be..e5d1e66`). Theo yêu cầu chủ dự án: trang Danh sách hóa đơn thu gọn thành công cụ chọn kỳ → xem SỐ ĐẾM → xuất/đồng bộ. (a) FilterBar trả lại 2 ô Từ ngày/Đến ngày (day-level, primitive `Field type=date`), GIỮ nút nhanh Tháng/Quý/Năm; (b) bỏ bảng danh sách + phân trang + chọn dòng + sắp/lọc cột + query `getInvoices` — số đếm lấy từ `/invoices/summary` (`total.count`); (c) bỏ dòng "Tổng thanh toán"; (d) bỏ caption "Kéo hóa đơn + dòng hàng từ Tổng cục Thuế…" ở panel đồng bộ. Hệ quả: bỏ theo cột Hàng hóa/Số lượng + xuất-dòng-đã-chọn (U30) — chủ dự án đã chốt. **(0) KHÔNG migration** — thuần `apps/web` (`git diff` chỉ đụng `apps/web/src|test/features/invoices*`; `packages`/`apps/api` không đổi ⇒ **CHỈ deploy `vat-web`**, khỏi `vat-api`). **⚠️ Working tree lúc commit còn deletion `docs/design/claude-design/*` KHÔNG phải của phiên này** (dấu hiệu hai-phiên-chung-thư-mục, bài học 2026-07-23) — đã **commit surgical `git add apps/web`**, KHÔNG dính deletion đó ⇒ bộ mockup Claude Design còn nguyên trên GitHub. **(1)** tsc + Biome sạch, **279 test xanh** (apps/web); thêm `filterBarDatePicker`, cập nhật `invoices`/`invoicesUK4`/`invoiceRangeSync`/`rangeBackfill`, xoá 4 test của bảng/chọn dòng/cột. **(2)** `git diff origin HEAD` **trống** trước build; `npm run build -w apps/web` (nạp `.env.production`) → bundle `index-C3c_q9SC.js`. **Chốt chặn (bài học 2026-07-22/23):** grep bundle — site key `0x4AAAAAAD6d5RwMpZVh-PZq` **CÒN** (1); code mới `Từ ngày`/`Đến ngày` **CÓ**; caption cũ `Kéo hóa đơn` **= 0** (bảo đảm build đúng code mới, không stale). **(3)** Deploy `vat-web` (Version `a67a5399-ba34-478b-8b46-35e83a57b377`; custom domain `vatengine.tourdao.vn`, binding `env.API`→vat-api + `env.ASSETS`). **(4) Smoke production:** SPA `/` **200** nạp đúng bundle mới; bundle LIVE — site key **còn**, `Từ ngày` **có**, `Kéo hóa đơn` **=0**; `/api/health` **200 `env=production`**; `POST /api/auth/login` (không captcha) → **400** ở tầng app (không 500/404 ⇒ front-door→vat-api sống, Turnstile vẫn bắt); **4/4 security header**. **CHƯA kiểm chứng bằng mắt end-to-end** (ranh giới thành thật): mở màn Hóa đơn thấy 2 ô ngày + chỉ số đếm + không bảng + không "Tổng thanh toán" — **cần đăng nhập tenant thật + captcha của chủ dự án** (Claude không giải captcha, không đăng nhập tenant). **CẢNH BÁO UX cho tenant thật:** deploy này **gỡ bảng xem hóa đơn** — kế toán sẽ xem qua file Excel (nút Xuất), không còn duyệt trên web.
>
