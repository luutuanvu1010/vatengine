...
## 0. Đường dẫn & bố cục repo (dọn 2026-07-21)

| | |
|---|---|
| Kho gốc | `/Users/tuanbao/Documents/Projects/vatengine` — **đổi tên từ `VATCrawlbot`**, đang ở trunk |
| Worktree đang có việc | `/Users/tuanbao/Documents/Projects/vat-u18` — nhánh `feat/u33-turnstile` |
| Thư mục cha | **`Documents/Projects`** (P hoa). Trên đĩa chỉ có MỘT thư mục; macOS không phân biệt hoa/thường nên cả hai cách gõ đều trúng, nhưng **viết `Projects`** cho nhất quán |

**Ba tên của cùng một thứ** — nguồn nhầm lẫn kinh niên: thư mục `vatengine` · kho GitHub `luutuanvu1010/vatengine` · sản phẩm `VATEngine`.

**Dọn 2026-07-21:** từ **17 worktree xuống 2**, thu lại **~5,7 GB**. Ba việc phát sinh khi dọn:
- Bốn tài liệu (thiết kế U32 đăng nhập Google, nghiên cứu VNeID, giới thiệu sản phẩm) nằm **untracked, không bản sao ở đâu** → đã cứu vào trunk (PR #31).
...
| ~~**QĐ-13**~~ | ~~Nhà cung cấp email = Resend~~ — **BỊ THAY THẾ bởi QĐ-16 cùng ngày** | 22-07 | Chủ dự án cho biết đã có hạn mức **SES 50.000 email/ngày** (đã thoát sandbox) ⇒ lý do chọn Resource ("dựng nhanh") không còn, mà nhược điểm trần 100/ngày thì có thật |
...
| **QĐ-16** | **Nhà cung cấp email = Amazon SES v2**, ký SigV4 bằng `aws4fetch`, đặt sau interface `EmailTransport` | 22-07 | Hạn mức 50.000/ngày đã có sẵn. SMTP bất khả thi từ Workers ⇒ đường HTTPS. SDK v3 quá nặng cho MỘT thao tác; tự viết SigV4 là mã ký mật mã, sai một bước thì lỗi hiện ra dưới dạng "403 không rõ lý do". ⚠️ `aws4fetch` bản mới nhất phát hành **2024-08-28** (đã tra npm 22-07) — SigV4 đã đóng băng nên nhiều khả năng là "xong việc", nhưng ghi nhận là rủi ro; nó nằm sau `EmailTransport` nên thay được bằng ~60 dòng. Xem **ADR-0007** |
| **QĐ-17** | **`audit_log` chỉ lưu SỰ KIỆN và THAM CHIẾU, KHÔNG lưu dữ liệu cá nhân** | 22-07 | Phát hiện 22-07: `maskSensitive` **không che email**, nên `chi_tiet` của `dang_ky` đang tích luỹ email khách dạng thô trong bảng bất biến — mỗi lượt đăng ký thêm một hàng không bao giờ xoá được. Danh tính tra ở `nguoi_dung` (ẩn danh hoá được) |
| **QĐ-18** | **KHÔNG BAO GIỜ `DELETE` hàng tenant — ẩn danh hoá tại chỗ**, trạng thái mới `da_xoa`, UNIQUE một phần trên `mst` để giải phóng | 22-07 | Xung đột trong sổ nợ vốn bị **đặt sai đề**: nó không nằm giữa `security.md` và NĐ 13/2023, mà giữa *xoá HÀNG* và *bất biến*. Quyền xoá dữ liệu đòi dữ liệu cá nhân không còn đọc được, KHÔNG đòi xoá hàng. Không `DELETE` ⇒ không cascade ⇒ trigger không kích hoạt ⇒ không còn gì để hoà giải. Xem **ADR-0007 §2** |
