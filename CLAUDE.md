...
Thứ tự triển khai: U0 (khung dự án + Makefile + CI) → U1–U3 (GDT Adapter) → U4–U5 (mô hình dữ liệu + đồng bộ idempotent) → U6–U7 (API tra cứu + kết xuất) → U8–U12 (đa tenant, đồng bộ nền, đối chiếu, kế toán, bảo mật) → **U13 (Giám sát rủi ro: contract định kỳ + probe egress; `docs/plans/EXP-giam-sat-rui-ro.md`)** → **U14 (Backend login/token GDT — API-only; `docs/plans/U14-design.md`)** → **U15 (Frontend — Tầng trình bày: SPA tra cứu/kết xuất/đối chiếu + màn Login, trên API nội bộ; `docs/plans/U15-plan.md`)** → **U16 (Frontend — Trang "Giới thiệu & Ủng hộ": mục đích phần mềm + góp ý Zalo/WhatsApp + đóng góp QR VietQR động theo mức; `docs/plans/U16-plan.md`)** → **U16b (Thiết kế lại trang "Giới thiệu & Hỗ trợ": ẩn Đóng góp bằng cờ, thêm Trung tâm hỗ trợ & tài liệu (FAQ) + Lịch sử cập nhật; `docs/plans/U16b-thiet-ke-lai-trang-gioi-thieu.md`)**.
