...
describe("RangeSyncPanel — render từng trạng thái (thuần tiến độ + cảnh báo)", () => {
  ...
  // không tạo trùng".
  it("idle + có tác vụ nền → hiện 'x tác vụ' + các kỳ + trấn an ghép vào phiên đang chạy", () => {
    panel(
      { kind: "idle" },
      {
        tacVuNen: {
    ...
    expect(screen.getByText(/tác vụ đồng bộ chạy nền cho kỳ/)).toBeInTheDocument();
    ...
    expect(screen.getByText(/ghép vào phiên đang chạy thay vì tạo phiên mới/)).toBeInTheDocument();
  });
  // Văn phong: không dùng chữ số trong câu văn xuôi ⇒ số ít viết chữ "một" (chốt 2026-07-30).
  it("idle + đúng MỘT tác vụ nền → viết chữ 'một', không viết chữ số 1", () => {
    panel(
      { kind: "idle" },
      {
        tacVuNen: {
  ...
  it("dang_lay (đã hiện tiến độ của chính phiên này) → KHÔNG lặp thêm dòng tác vụ nền", () => {
    ...
    expect(screen.queryByText(/tác vụ đồng bộ chạy nền/)).not.toBeInTheDocument();
  });
