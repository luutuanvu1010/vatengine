...
// 10 = doanh nghiệp/tổ chức; 10-3 (gạch ngang, chuẩn TT 105/2020, vd 0305097236-005)
// hoặc 13 số liền = đơn vị phụ thuộc (chi nhánh); 12 = số định danh cá nhân
// (hộ kinh doanh / cá nhân, theo TT 86/2024/TT-BTC). Khớp `apps/api/.../dangKy.ts`.
const MST_RE = /^\d{10}(-\d{3})?$|^\d{12}$|^\d{13}$/;

// Một nguồn cho CẢ thông điệp lỗi server lẫn nhắc tại chỗ — hai chỗ hiển thị không lệch nhau.
const MST_SAI_DANG =
  "Mã số thuế phải gồm 10, 12 hoặc 13 chữ số; chi nhánh dùng dạng 10 số, gạch ngang và 3 số (ví dụ 0305097236-005).";
        ...
        {xong ? (
          ...
          <Card>
            ...
            <form onSubmit={guiForm} style={{ display: "grid", gap: "var(--sp-4)" }}>
              ...
              />
              {/* KHÔNG dùng inputMode="numeric": bàn phím số trên di động thiếu phím "-",
                  chi nhánh (0305097236-005) sẽ không gõ nổi mã của chính mình. */}
              <TextField
                label="Mã số thuế"
                required
                value={mst}
                onChange={(e) => setMst(e.target.value)}
                placeholder="Ví dụ: 0305097236 hoặc 0305097236-005"
