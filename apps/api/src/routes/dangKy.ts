...
export function dangKyRoutes(deps: AppDeps) {
  ...
  r.post("/", async (c) => {
    // C — kiểm limiter TRƯỚC KHI mở kết nối DB. Đây là điểm dễ bị dội tải nhất (endpoint
    // công khai, không xác thực) — mở kết nối DB cho MỌI request kể cả rác chính là thứ
    // limiter tồn tại để ngăn.
    ...
    try {
      ...
      // B — record() gọi ĐÚNG MỘT LẦN, và CHỈ SAU khi đã qua cổng check() ở trên (không
      // bao giờ record() một request đã biết là bị chặn — tránh thổi phồng cửa sổ đếm).
      // finally đảm bảo tính đủ MỌI lượt dưới ngưỡng (thành công, 400 nghiệp vụ, 409 trùng
      // lặp) đúng tinh thần A: một kẻ lạm dụng gửi rác hàng loạt vẫn phải bị đếm dù mọi lượt
      // đều thất bại ở tầng validate — nếu không đếm, validate rẻ (không chạm DB) trở thành
      // đường né limiter hoàn toàn.
